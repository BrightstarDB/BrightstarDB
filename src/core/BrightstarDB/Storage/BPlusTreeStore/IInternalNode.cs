...
namespace BrightstarDB.Storage.BPlusTreeStore
{
    internal interface IInternalNode : INode
    {
        ...
        bool Merge(ulong txnId, INode s, byte[] joinKey);
        ...
        /// </summary>
        /// <param name="txnId">The transaction id for this update</param>
        /// <param name="rightNodePage">The page reserved to receive the newly created internal node</param>
        /// <param name="splitKey">Receives the value of the key used for the split</param>
        /// <returns>The new right-hand node</returns>
        IInternalNode Split(ulong txnId, IPage rightNodePage, out byte[] splitKey);
        ...
        void Insert(ulong txnId, byte[] key, ulong childPointer);
        ...
        void UpdateChildPointer(ulong txnId, ulong oldChildPointer, ulong newChildPointer);
        ...
        void SetLeftKey(ulong txnId, ulong childNodeId, byte[] childNodeKey);

        byte[] GetKey(ulong childNodeId);

        void SetKey(ulong txnId, ulong childNodeId, byte[] newKey);
        ...
        byte[] RemoveChildPointer(ulong txnId, ulong childNodeId);
